...
class FinancialRiskAgent:
    def __init__(self):
        ...
        # 本地股票名称到代码的映射字典，作为API调用失败时的备用
        self.local_stock_map = {
            "贵州茅台": "600519",
            "宁德时代": "300750",
            "紫金矿业": "601899",
            "海康威视": "002415",
    ...
    def stock_name_to_code(self, name_or_code):
        ...
        # 优先检查本地映射
        if name_or_code in self.local_stock_map:
            return self.local_stock_map[name_or_code]
        
        # 尝试通过akshare获取股票代码，兼容不同版本
        try:
            try:
                # 新版本接口
    ...
    def get_stock_basic_info(self, code):
        ...
        # 暂时禁用缓存，确保每次都重新获取股票名称
        # 后续可以考虑优化缓存策略，只缓存有效的股票名称数据
        ...
        try:
            ...
            # 获取股票名称
            # 1. 首先检查本地映射，创建反向映射以提高查找效率
            local_reverse_map = {v: k for k, v in self.local_stock_map.items()}
            stock_name = local_reverse_map.get(code, code)  # 默认使用代码作为名称
            
            if stock_name != code:
                print(f"从本地反向映射获取到股票名称：{stock_name}")
            else:
                # 2. 使用格式化的股票代码查询baostock
                rs_name = bs.query_stock_basic(code=formatted_code)
                
                # 直接使用baostock API获取股票名称
                if rs_name.error_code == '0':
                    while rs_name.next():
                        ...
                        if len(name_row) > 1:
                            # 通常股票名称在第二个字段
                            if name_row[1] and name_row[1].strip():
                            ...
                            for i, field in enumerate(name_row):
                                if field and field.strip() and not field.isdigit() and '.' not in field:
                                    stock_name = field
                                    print(f"找到股票名称：{stock_name}（索引：{i}）")
                                    break
                            break
            
            # 3. 如果仍然没有找到名称，尝试使用akshare作为备用方案
            if stock_name == code:
                print(f"尝试使用akshare获取股票名称：{code}")
                try:
                    import akshare as ak
                    # 使用正确的akshare函数获取股票代码到名称的映射
                    # 兼容不同版本的akshare
                    try:
                        # 优先使用最新的函数
                        stock_info = ak.stock_info_a_code_to_name()
                        print(f"akshare函数 stock_info_a_code_to_name 调用成功")
                    except AttributeError:
                        # 旧版本函数
                        stock_info = ak.stock_info_a_code_name()
                        # 转换为字典格式：{code: name}
                        stock_info = dict(zip(stock_info['code'], stock_info['name']))
                        print(f"akshare函数 stock_info_a_code_name 调用成功")
                    
                    # 检查6位数字代码是否在映射中
                    if code in stock_info:
                        stock_name = stock_info[code]
                        print(f"akshare获取到股票名称：{stock_name}")
                    else:
                        # 尝试添加市场前缀后查找
                        print(f"akshare未找到股票名称：{code}，尝试其他方法")
                        # 直接使用akshare的股票查询功能
                        try:
                            # 获取所有A股列表
                            stock_list = ak.stock_info_a_spot_em()
                            # 查找匹配的股票
                            matched_stock = stock_list[stock_list['code'] == code]
                            if not matched_stock.empty:
                                stock_name = matched_stock.iloc[0]['name']
                                print(f"使用stock_info_a_spot_em获取到股票名称：{stock_name}")
                        except Exception as e:
                            print(f"使用stock_info_a_spot_em获取失败：{e}")
                            # 最后尝试本地映射的反向查找
                            for name, stock_code in self.local_stock_map.items():
                                if stock_code == code:
                                    stock_name = name
                                    print(f"从本地映射获取到股票名称：{stock_name}")
                                    break
                except Exception as e:
